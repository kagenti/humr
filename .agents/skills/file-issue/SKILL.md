...
Draft a GitHub issue and file it after the user approves. For the content shape — what belongs in an issue, what doesn't, and the template — use the `draft-issue` skill. This skill layers the workflow (understand → research → dedupe → draft → approve → file) on top of those guidelines.
...
4. **Draft inline.** Produce the draft following the `draft-issue` skill (template + guidelines). Present the full draft (title + body) in the chat. Do not file yet.
