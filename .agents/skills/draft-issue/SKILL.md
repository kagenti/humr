...
1. **Understand the request.** Restate the problem back to the user in one or two sentences if it's ambiguous. Ask follow-ups only for things that would change the shape of the issue (scope, who it affects, dependencies on other work).

2. **Research only as much as needed.** You may explore the codebase to learn domain vocabulary and how the feature is perceived today — enough to write accurately about the *user-visible* status quo. Do not pull file paths, function names, or architectural detail into the issue itself.
