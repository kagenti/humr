...
export function EditAgentSecretsDialog({
  ...
  const [initialAssignedAppIds, setInitialAssignedAppIds] = useState<string[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setApps(appList);
        const connIds = [...agentApps.connectionIds].sort();
        setAssignedAppIds(new Set(connIds));
        setInitialAssignedAppIds(connIds);
      } catch (err: any) {
