...
Some connections also need *files* in the pod (e.g. the GitHub Enterprise host entry in `~/.config/gh/hosts.yml`). For those, humr ships a separate file-push mechanism: a `humr-config-sync` sidecar holds an SSE connection to the api-server and merges declared file fragments into a shared `emptyDir` without rolling the pod. The agent never sees a real upstream credential — the sidecar writes the same `humr:sentinel` placeholder used in env vars, and the gateway swaps it on outbound requests.
