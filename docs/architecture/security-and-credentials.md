...
- **Named OAuth apps** (GitHub, GitHub Enterprise) drive the same auth-code+PKCE engine as the MCP flow. Each user supplies their own OAuth client id + secret (and GHE host where applicable) through a connect form — the platform does not ship default client credentials, and there is no admin chart configuration for these. The Connections UI's "Connect GitHub / GitHub Enterprise" button replaces OneCLI's dashboard entry point; new connections always land in the K8s Secret first and are mirrored into OneCLI as a generic secret named `__humr_oauth:<connection>` with `Authorization: Bearer {value}` injection on the app's host pattern. OneCLI's app-connection list (provider-driven OAuth that never left OneCLI's UI) remains visible in the UI as a read-only section until those providers move over too. A future slice may layer in a public default client id or RFC 7591 dynamic client registration where the provider supports it.
