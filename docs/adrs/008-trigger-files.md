...
The Controller delivers triggers via `kubectl exec`, writing a JSON file into `/workspace/.triggers/{timestamp}.json` inside the running pod. The harness watches that directory and creates a new ACP session per trigger file, then deletes the file after processing.
