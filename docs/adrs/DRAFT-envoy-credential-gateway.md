...
- **Storage.** API Server writes a Kubernetes Secret labeled by instance instead of `POST`ing to OneCLI's REST API. The owning pod's ServiceAccount governs read access. Envoy SDS picks up changes without a pod restart.
